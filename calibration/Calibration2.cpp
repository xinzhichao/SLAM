...
int main()
{
    ...
    for (int i = 0;  i<9; i++)
    {
        ...
        Mat imageInput = cv::imread((fmt  % "calibration" % "images" % (i + 1) % "jpg").str());
        //filenames.push_back(filename);
        cout << fmt <<endl;
        filenames.push_back(cv::imread((fmt % "calibration"% "chess" % (i + 1) % "jpg").str()));
        // 读入第一张图片时获取图片大小
    ...
    //-------------以下是摄像机标定------------------
    /*棋盘三维信息*/
    Size square_size = Size(26.5, 26.5);         /* 实际测量得到的标定板上每个棋盘格的大小 */
